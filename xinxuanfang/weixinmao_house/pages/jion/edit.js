...
Page({
  ...
  toTap: function (e) {
    ...
    this.data.ptitle = type == 'video' ? '视频上传' : '资料修改';
  ...
  onLoad: function (e) {
    var a = this;
    if ($.isObj(e) && e.type == 'video') {
      this.data.type = e.type; this.data.ptitle = '视频上传';
    }
    a.data.id = e.id, wx.setNavigationBarTitle({
      title: this.data.ptitle + "-" + wx.getStorageSync("companyinfo").name
    });
      ...
      success: function (e) {
        if (!e.data.message.errno) if (e.data.data.intro.maincolor || (e.data.data.intro.maincolor = "#3274e5"),
          wx.setNavigationBarColor({
            ...
            upmax: e.data.data.upmax || [],
            uptitle: e.data.data.uptitle || [],
            updesc: e.data.data._desc || []
          }), 1 == e.data.data.error) {
          a = e.data.data.msg;
          wx.redirectTo({
  ...
  savepubinfo: function (a) {
    ...
    for (var _i in t.data.upimgs[t.data.type]) {
      imgs.push(t.data.upimgs[t.data.type][_i].s || '');
    }
    imgs = imgs.toString();
  ...
  bindUpload: function (e) {
    ...
    var th = this, status = th.data.data.status;
    if (!(status >= 0) || (!(status >0)&&th.data.type=='video'))return;
    var method = th.data.type == 'video' ? 'chooseVideo' :'chooseImage'
    wx[method]({
      count: 1,
      sizeType: ['original', 'compressed'],
      sourceType: th.data.type == 'video' ? ['album']:['album', 'camera'],
      success(res) {
        var p = $.isArr(res.tempFilePaths)? res.tempFilePaths[0]: res.tempFilePath;
        var max = th.data.upmax[th.data.type];
        var file = res.tempFiles?res.tempFiles[0]:res;
        if (max * 1024 < file.size){
          max = Math.floor(max / 1024);
          return $.alert('文件大小不能超过 ' + (Math.max(max,max-10))+' M');
        }
        t.doUpload({
          path: p,
          success: function (e) {
            if (e.data.data.path) {
              p = th.data.type == 'video' ?'../../resource/icon/video.png':p;
              th.setData({
                ['upimgs.' + th.data.type+'.[' + i + ']']: { p: p, s: e.data.data.path }
              });
            }
          }
        });
  ...
  bindDelete: function (e) {
    var t = app, i = e.currentTarget.dataset.idx,type=this.data.type;
    var file = this.data.upimgs[type] || [];
    var f = (file[i] || {}).s;
    var p = (file[i] || {}).p;
    this.data.upimgs[type][i]=[];
    this.setData({
