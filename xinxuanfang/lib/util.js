  ...
  if (isTab&&j.fail === undefined && j.url && j.url != '') {
    ...
    j.fail = function (r) {
      ...
      if (errMsg.indexOf(' a tabbar page') > -1 || errMsg.indexOf(' a tab bar page') > -1){
        ...
        var method ='navigateTo',s=','+j.switch+',';
...
util.toTab=util.switchTab = function (e,tab) {
  ...
  if (j.url == '' || !j.url) { console.info('no url'); return };
  wx.switchTab(j);
}
...
util.toMap = util.openLocation=function(e){
  ...
  if (!t) { console.info('no latlng'); return };
  j = this.extend(j, t);
  if (j.latlng){
...
util._s2q=function(s,arg,ext){
  ...
  var e = {}, args ={'miniapp': ['appId', 'path','envVersion']};
  var extk ='extData';
  if (ext && this.isStr(ext)){
    var exts = (ext + ',').split(','), extk = exts[1].trim(), ext = exts[0].trim();
  ...
  for(var i in j){
    ...
    var k=v[0].trim();v.shift(),v.pop(),v=v.join(':');
    if (v=='false'){
      v=false;
    }
    if(v=='true'){
...
util.showLoading=function(e,s){
  var j = { icon: 'loading', duration:600000};
  if (s && this.isNumber(s)) {
    j.duration = s;
  }
  var t = this.isBool(e)?{mask:e}: this._s2qe(e,'title,mask,duration');
  if (!t) t = {};
  if (this.isBool(t.title)) {
    if (t.mask == undefined) {
      t.mask = t.title; delete t.title;
    }
  }
  t.title==undefined?t.title='加载中':'';
  j = this.extend(j, t, { icon: 'loading' });  
...
util.showImg = util.previewImage=function(e,urls){
  ...
  if (urls&&this.isArr(urls)&&(this.isNumeric(e)||this.isStr(e))){
    t={urls:urls,current:e};
  }else{    
    t = this.isArr(e) ? { urls: e } : this._s2qe(e, t='urls,current,curr', t);
  }
...
util.saveFile=util.saveFiles = function (e,type) {
  ...
  t = this.isArr(e) ? { urls: e } : this._s2qe(e, t = 'urls,filePath', t);
  if (!t) return;
  j = this.extend(j, t);
  type = type||j.type||'file';
  type = ['img', 'video', 'file'].indexOf(type.toLowerCase()) >= 0 ? type : 'file';
  var method = 'saveFile', tmsg = '文件', dw = '个';
  ...
  var succ, fail, complete, com, l = urls.length, s = 0, er = 0;
  com = function (r, t) {
    if (s + er < l) return;
    var msg = '保存成功';
  ...
  var run = function () {
    ...
    for (var i in urls) {
      if (!urls[i]) { l--; continue; }
      wx.downloadFile({
        url: urls[i],
        success: function (r) {
          if (r.statusCode === 200) {
            var jj = $.extend(true, {}, j, { [tempFilePath]: r.tempFilePath });
            wx[method](jj);
          } else {
            j.fail(r);
          }
        },
        fail: function (r) { j.fail(r); }
      });
  ...
  return wx.authorize({ scope: 'scope.writePhotosAlbum', fail() { $.toast('您已拒绝授权保存'+tmsg); }, success: function () { run(); } });
}
util.scan = util.scanCode=function(){
  var i = 0, j = {}, e = {}, arg = arguments, l = arg.length-1;
  do{
    if (l < i) return;
    if ($.isBool(arg[i])) {
...
util.authorize = function (e, succ, fail, complete){
  ...
  if (fail === true && !this.isFun(j.fail)) j.fail = function (r) { $.alert('您已拒绝使用位置信息，若需位置服务请打开设置界面（「右上角」 - 「设置」）开启位置服务授权'); };
  if ((this.isObj(fail) || this.isStr(fail))&& !this.isFun(j.fail))j.fail=function(){$.toUrl(fail);}
...
util.getLocation = function (e, succ, fail, complete){
  ...
  if (!this.isFun(j.fail) && this.isFun(fail)) j.fail = fail;
  if (!this.isFun(j.complete) && this.isFun(complete)) j.complete = complete;
  if (!this.isFun(j.fail)) j.fail = function (r) { $.alert('获取位置失败，请开启GPS服务'); }; 
  if ((this.isObj(fail) || this.isStr(fail)) && !this.isFun(j.fail)) j.fail = function () { $.toUrl(fail); }
  return this.authorize('scope.userLocation',function() { wx.getLocation(j)},true);
}
util.chooseLocation = function (e, succ, fail, complete) {
  ...
  return this.authorize('scope.userLocation', function () { wx.chooseLocation(j) }, true);
}
util.loadPage=function(e){
  var app = getApp(), e = this.isObj(e) ? e : {}; e = this.extend(true, e, app.load('/lib/page.js'), this.extend(true,{},e));
  return e;
}
...
util.append = function (p, key, append, page, data) {
  ...
  var d = this.isArr(data) ? this.extend(true, [], data) : [], a = this.isArr(append) ? this.extend(true, [], append) : [];
  if(!(page>1))return p.setData({[key]:a});
  if(a.length<=0)return;
  var sets = {}, l = parseInt(d.length);
  for(var i in a){
    i=parseInt(i);
    sets[key+'['+(i+l)+']']=a[i];
  }
